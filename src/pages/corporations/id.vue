    ...
    <div class="container">
      <div class="inner-page__inner">
        <Breadcrumb>
          ...
          <li class="breadcrumb-item">
            <router-link to="/corporations">Корпорации</router-link>
          </li>
          <li
            class="breadcrumb-item active"
            aria-current="page"
            v-if="card && card.public_name"
          >
            {{ card.public_name }}
          </li>
        </Breadcrumb>
        <p class="title" v-if="card && card.public_name">
          {{ card.public_name }}
        </p>
...
export default {
  ...
  setup() {
    ...
    const card = computed(() => store.state.search.corporation);

    const InitData = () => {
      store.dispatch("search/getCorporation", route.params.id).then((res) => {
