    ...
    <div class="container">
      <div class="inner-page__inner">
        <Breadcrumb>
          ...
          <li class="breadcrumb-item">
            <router-link to="/investors">Инвесторы</router-link>
          </li>
          <li
            class="breadcrumb-item active"
            aria-current="page"
            v-if="card && card.contact_name"
          >
            {{ card.contact_name }}
          </li>
        </Breadcrumb>
        <p class="title" v-if="card && card.contact_name">
          {{ card.contact_name }}
        </p>
...
export default {
  ...
  setup() {
    ...
    const card = computed(() => store.state.search.investor);

    const InitData = () => {
      store.dispatch("search/getInvestor", route.params.id).then((res) => {
